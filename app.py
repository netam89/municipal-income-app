...
# המרת עמודת אשכול למספר
df_clean['אשכול'] = pd.to_numeric(
    df_clean['אשכול מדד חברתי כלכלי (משנת 2019, מ-1 עד 10, 1 הנמוך ביותר)'], errors='coerce'
)

# מחשבים ממוצעים לפי אשכול
grouped = df_clean.groupby('אשכול')[income_columns].mean().reset_index()

# מוצאים את ערכי הרשות הנבחרת
selected_row = df_clean[df_clean['שם הרשות'] == selected_city].copy()

if not selected_row.empty:
    selected_group = selected_row.iloc[0]['אשכול']
    ...
    ax.set_xticklabels([str(int(val)) for val in grouped['אשכול']])
    ax.legend(loc='best')
